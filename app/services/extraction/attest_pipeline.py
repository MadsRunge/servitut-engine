    ...
    *,
    ...
    while start < len(page_blocks):
        ...
        segment = AttestSegment(
            ...
            raw_matrikel_references=_parcel_refs_from_text(segment_text),
        )
...
def _load_or_build_pipeline_state(
    ...
    existing = storage_service.load_attest_pipeline_state(case_id, doc_id)
    if (
        existing
    ...
    storage_service.save_attest_pipeline_state(case_id, doc_id, state)
    _emit_progress(
        progress_callback,
        doc_id=doc_id,
        source_type="tinglysningsattest",
...
def _merge_attest_servitut(existing: Servitut, incoming: Servitut) -> Servitut:
    ...
    return existing.model_copy(
        update={
            ...
            "akt_nr": existing.akt_nr or incoming.akt_nr,
            "title": incoming_title
            if incoming_title and len(incoming_title) > len(existing_title)
            else existing.title,
            "applies_to_matrikler": _unique_preserve_order(
                [*existing.applies_to_matrikler, *incoming.applies_to_matrikler]
            ),
            "raw_matrikel_references": _unique_preserve_order(
                [*existing.raw_matrikel_references, *incoming.raw_matrikel_references]
            ),
...
def _find_merge_candidate(merged: list[Servitut], incoming: Servitut) -> Optional[int]:
    ...
    incoming_akt = _normalize_key(incoming.akt_nr)
    ...
    if incoming_akt:
        for idx, existing in enumerate(merged):
            if _normalize_key(existing.akt_nr) == incoming_akt:
                return idx
    if incoming_title:
        for idx, existing in enumerate(merged):
            if (
...
def merge_attest_servitutter(servitutter: List[Servitut]) -> list[Servitut]:
    ...
    for servitut in servitutter:
        ...
        if match_index is None:
            merged.append(
                servitut.model_copy(
                    update={"scope_source": servitut.scope_source or "attest"}
                )
            )
            continue
